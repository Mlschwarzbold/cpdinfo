...
vector<string> allCommands{"exit", "help", "player", "user", "tags", "top", "eastergg"};
...
Operation parseInput(string input, set<string> allTags) {
    ...
    if (found != string::npos) {
      ...
      if (operationString.compare("help") == 0) {
            opCode = HELP_TEXT;
            cout << "Available commands are: " << endl << "          player <name> -> search for player name or prefix" << endl << "          user <ID> -> search for user reviews, by ID" << endl << "          tags <tags> -> find players by their tags " << endl << "          top<n> <position> -> top N players for a position" << endl << "          help -> displays this help menu" << endl;
