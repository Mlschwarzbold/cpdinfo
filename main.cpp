...
int main() {
  ...
  ifstream infoF("players_21.csv");
  aria::csv::CsvParser infoParser(infoF);
  ...
  int ir = 0;
  for (auto& infoRow : infoParser) {
    if(ir == 0){
      for(int i = 0; i < 106; i++){
        allNames.push_back(infoRow[i]);
      }
    } else {
        tempInfo.clear();
        for(int i = 0; i < 106; i++){
          tempInfo.push_back(infoRow[i]);
        }
  ...
  duration = duration_cast<TIMEUNIT>(endTime - startTime);
  totalTime += duration;
  cout << "Finished adding info dump " << duration.count() << "ms" << endl;
